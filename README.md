...
Content (services, portfolio case studies, blog posts) lives in `app/data/*.ts` as typed arrays, no CMS or database. Edit those files directly to update copy. Portfolio case study images are currently sourced from Unsplash by URL; swap in real project photography when available.
...
> **Windows/Node note:** This project's SEO tooling (`@unhead/bundler`) requires Node's `require(esm)` support. The `dev`/`build`/`generate` scripts already set `NODE_OPTIONS=--experimental-require-module` via `cross-env` for compatibility with Node 22.6 through 22.11. On Node 22.12+ this flag is a no-op (the feature is already default-on).
...
- **Portfolio images:** `image` fields in `app/data/portfolio.ts` currently point to Unsplash stock photos. Replace with real project screenshots or photography.
...
```
app/
  assets/css/main.css   Tailwind theme tokens (colors, fonts, reveal animation)
  ...
  plugins/reveal.client.ts      Scroll-in-view fade/slide animation directive (v-reveal)
server/
